...
# Some variables we'll use here
    our ($dest, $format, $usage, $underscores, $live, $subcat, $rename, $maxlength);
...
--subcategory
    ...
    Setup Movies and TV Shows/Episodes into their own sub-folders (i.e. Movies/title and Episodes/Show/title)

    default: do not include this sub-category folder
...
--format
    ...
    \%C   = Category
    \%U   = RecGroup
    \%Se  = Season (always 2 digits, i.e. 02)
    \%Ep  = Episode (always 2 digits, i.e. 02)
    ...
    \%se  = Season & Episode information (i.e. S03E01)
    \%%   = a literal % character
...
# Create symlinks for the files on this machine
	...
	my $prog = new MythTV::Program(@$row);
	my $season = $show->{'season'};
	my $episode = $show->{'episode'};
	my $seriesid = $show->{'seriesid'};
	my $cat = $show->{'category'};
	my $stars = $show->{'stars'};
	my $airdate = $show->{'airdate'};
	my $progid = $show->{'programid'};
	my $sep = $show->{'syndicatedepisode'};
	vprint("name: $show->{'title'}");
	vprint("SHOW synd_ep: $sep | season: $season.$episode | seriesID: $seriesid | category: $cat | stars: $stars | airdate: $airdate | programID: $progid");
	# vprint($prog);
	my $p_season = $prog->{'season'};
        my $p_episode = $prog->{'episode'};
        my $p_seriesid = $prog->{'seriesid'};
        my $p_cat = $prog->{'category'};
        my $p_stars = $prog->{'stars'};
	my $p_inet = $prog->{'inetref'};
	my $p_airdate = $prog->{'airdate'};
	my $p_progid = $prog->{'programid'};
	my $p_sep = $prog->{'syndicatedepisode'};
	# vprint("PROG synd_ep: $p_sep | season: $p_season.$p_episode | seriesID: $p_seriesid | category: $p_cat | stars: $p_stars | inet: $p_inet | airdate: $p_airdate | programID: $p_progid");
	my $type = "Episodes";
	$progid =~ /([a-zA-Z][a-zA-Z])*/;
	if ($1 eq "MV") {
                $type = "Movies";
                vprint("Movie!");
	}

    # If the season and episode information is not in place, figure it out from syndicated episode info
	if ($season == 0 || $episode == 0) {
		$sep =~ /([0-9]{1,})([0-9]{2})/;
		if (defined($sep) && $sep ne "" && defined($1) && defined($2)) {
			vprint("Figure out episode from syndicate episode($sep) (S$1E$2)");
			$show->{'season'} = $1;
			$show->{'episode'} = $2;
		} else {
			vprint("whacked value: $sep (S:$season E:$episode)");
			$show->{'season'} = 99;
			$show->{'episode'} = 99;
		}
	}
